...
using Spiffy.Plugins.Popup.Contracts;
using Spiffy.Plugins.Popup.Pages;
using Xamarin.Forms;

namespace Spiffy.Plugins.Popup.Services
{
        ...
        private IPopupPlatform PopupPlatform
        {
            get
            {
                ...
                if(popupPlatform == null)
                    throw new InvalidOperationException("You MUST install Spiffy.Plugins.Popup to each project and call Spiffy.Plugins.Popup.Popup.Init(); prior to using it.\nSee more info: " + Config.InitializationDescriptionUrl);

                if(!popupPlatform.IsInitialized)
                    throw new InvalidOperationException("You MUST call Spiffy.Plugins.Popup.Popup.Init(); prior to using it.\nSee more info: " + Config.InitializationDescriptionUrl);
